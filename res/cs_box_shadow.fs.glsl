...
void main(void) {
    vec2 pos = vLayerPos.xy;
    vec2 p0Rect = vBoxShadowRect.xy;
    vec2 p1Rect = vBoxShadowRect.zw;

    bool in_rect = pos.x > vSrcRect.x && pos.x < vSrcRect.z &&
                   pos.y > vSrcRect.y && pos.y < vSrcRect.w;
    if ((in_rect && vInverted == 0) ||
        (!in_rect && vInverted == 1)) {
        discard;
    }
    ...
    oFragColor = vec4(vColor.rgb, max(value, 0.0));
}
